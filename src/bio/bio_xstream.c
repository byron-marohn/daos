...
get_spdk_log_page_completion(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg)
{
	...
	/*
	 * status code = INTERNAL_DEVICE_ERROR = 6
	 * type: 0 = SPDK_NVME_SCT_GENERIC
	 */
}
...
/*	uint32_t					 i; */
	...
	uint32_t					 numd, numdl, numdu, lpol, lpou;
	int						 rc;

	 if (ctxt->bxc_health_stat_age + DAOS_SPDK_STATS_PERIOD >= now)
		return;

	/* used example in spdk_nvme_ctrlr_cmd_get_log_page() to set cmd parameters */
	payload_size = sizeof(health_page);
	if (payload_size == 0)
		return;
	...
	if (ctxt->bxc_desc != NULL) {
		...
		/* example from spdk_rpc_apply_nvme_firmware() */
		/* check whether the block device supports admin I/O before passthru */
		if (!spdk_bdev_io_type_supported(bdev, SPDK_BDEV_IO_TYPE_NVME_ADMIN)) {
			D_ERROR("SPDK_BDEV_IO_TYPE_NVME_ADMIN not supported\n");
			goto cleanup;
		}

		/*
		 * Use spdk_bdev_nvme_admin_passthru() to submit an NVMe admin
		 * command to bdev.
		 */
		rc = spdk_bdev_nvme_admin_passthru(ctxt->bxc_desc,
						   channel,
						   cmd,
						   &health_page,
						   sizeof(health_page),
						   ...
						   NULL);
		if (rc) {
			D_ERROR("Error submitting bdev nvme admin passthru command: %d\n",
				rc);
			goto cleanup;
		}
	}
...
*/
	 ...
	 */
	D_PRINT("==========================================================\n");
	D_PRINT("SPDK Device [%s] Health Information:\n",
		bdev != NULL ? spdk_bdev_get_name(bdev) : "");
	D_PRINT("==================================\n");
